"""Stage 3.6(D0079): `financial_quality_metric_to_evidence_market_public_at()`のTest。

Cash Flow(CFO/CFI/CFF)専用のA系統(MARKET_PUBLIC_AT)Evidence Converterが、
既存`source_version_to_evidence_market_public_at()`と同じPIT Semanticsを
維持しつつ、period_start/period_end等のMetadataを失わないことを確認する。
