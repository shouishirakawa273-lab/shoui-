...
| PIT semantics | `DiscDate`/`DiscTime`(Asia/Tokyo)を`market_public_at`として使用(`AvailabilityBasis.EXACT`、D0048、EXTERNAL_OFFICIAL_SPEC_VERIFICATION §7で確認と申告)。ただし`provider_available_at`は実際の観測ログが無い限り常に`AvailabilityBasis.UNKNOWN`のまま(Fallbackしない)。今日取得したBulk/List Dataから過去の`provider_available_at`を復元することは禁止(Historical Market Time vs Historical Provider Time、`TDNET_ARCHITECTURE.md` §3) |
