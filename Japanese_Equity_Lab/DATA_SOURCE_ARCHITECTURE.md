...
| PIT semantics | `DiscDate`/`DiscTime`(Asia/Tokyo)から`market_public_at`の値を構築(D0048、EXTERNAL_OFFICIAL_SPEC_VERIFICATION §7で確認と申告)。ただしBasisは`AvailabilityBasis.EXACT`ではなく`AvailabilityBasis.UNKNOWN`のまま維持する(skeptic-reviewer Finding、D0048追記 — 確認根拠が又聞きでありClaude自身の直接確認・真のLocal Real Data Validationのいずれでもないため。`disclosures_as_of()`の既定安全側除外がそのまま適用される)。`provider_available_at`も実際の観測ログが無い限り常に`AvailabilityBasis.UNKNOWN`のまま(Fallbackしない)。今日取得したBulk/List Dataから過去の`provider_available_at`を復元することは禁止(Historical Market Time vs Historical Provider Time、`TDNET_ARCHITECTURE.md` §3) |
| Current Implementation Status | NOT_IMPLEMENTED(Adapter/Normalizerコードは実装済みだが、EXTERNAL_OFFICIAL_SPEC_VERIFICATIONに基づくものであり真のLocal Real Data Validation未実施のため`CONNECTED`へは未昇格、D0048)。Raw Fetch: `lib.disclosures.providers.tdnet.TdnetAdapter`(`/v2/td/list`・`/v2/td/files`・`/v2/td/bulk`)。Normalize: `lib.disclosures.providers.tdnet_normalize`。Retrieval Cursor Provenanceの State Model骨格: `lib.disclosures.providers.tdnet_cursor.TdnetRetrievalCursorState`(D0047、Adapterとは`extract_retrieval_cursor_fields()`経由で接続) |
| Cost/Plan dependency | 月額11,000円(税込)、Lightプラン以上へのAdd-on(EXTERNAL_OFFICIAL_SPEC_VERIFICATION、ユーザー申告、D0048)。Current Userがこのアドオンを契約済みとは仮定しない(この申告自体もClaude自身が一次資料をFetchして確認したものではない) |
| Known limitations | D0047時点でPhase4B-3のSource Onboarding調査(`TDNET_SOURCE_ONBOARDING.md`)は、本セッションから`jpx-jquants.com`・`jpx.gitbook.io`いずれへも接続できず(egressポリシーによりブロック)、タスク上最重要の2項目(`DiscStatus`/`RevNo`の訂正・削除意味論、`DiscDate`/`DiscTime`のPIT意味論)の裏付けがゼロだった。D0048でユーザーが別のWeb-access環境からの直接確認を申告し(EXTERNAL_OFFICIAL_SPEC_VERIFICATION)、これを根拠にAdapter/Normalizerを実装した。ただし: (1) この申告はClaude自身の直接確認でも真のLocal Real Data Validationでもない。(2) BASE_URL(`api.jquants.com/v2`共有)・Response Envelope形状(`{"data": [...]}`)はMain Claudeによる推論。(3) `DiscItems`公式Code List・`Docs`のg/s/x以外の詳細意味論は依然未確認、`document_kind`は常に`UNKNOWN`。(4) `DiscStatus`は現在常にnull・`RevNo`は現在常に1という「現在の実装挙動」を、正規化済みRecordのLifecycle判定には使用しない(Provider宣言Schema意味論とは別軸として保持のみ)。(5) 新DiscNoによる旧DiscNoの訂正Relationshipは自動生成しない。詳細は`TDNET_SOURCE_ONBOARDING.md`・`TDNET_ARCHITECTURE.md`・`DECISIONS.md` D0047/D0048参照 |
