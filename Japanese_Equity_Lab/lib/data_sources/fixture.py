"""テスト・ローカルPipeline検証用の DataSourceAdapter 実装。

実際のJ-Quants等への通信を行わず、あらかじめ用意したfixtureデータ(JSON)を
J-Quantsのペイロードと同じ構造で返す。**合成データであり、実際の株価ではない。**
...
RESPONSE_SCHEMA_VERSION = "fixture-v1(synthetic)"


class FixtureDataSourceAdapter:
    """fixture JSONファイルから日次株価・取引カレンダーを返す(合成データ専用)。

    fixtureファイルの構造は
    ``{"daily_quotes": {code: [{"Code":..,"Date":..,"Open":..,"High":..,"Low":..,
    "Close":..,"Volume":..}, ...]}, "trading_calendar": [{"Date":..,"HolidayDivision":..}]}``
    を想定する(J-Quantsのフィールド名を模す)。
    """
    ...
    def fetch_daily_quotes(self, *, codes: Sequence[str], start_date: date, end_date: date) -> RawFetchResult:
        all_quotes: dict[str, list[dict[str, Any]]] = self._data.get("daily_quotes", {})
        records: list[dict[str, Any]] = []
        for code in codes:
            for row in all_quotes.get(code, []):
